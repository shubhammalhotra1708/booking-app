...
export default function SalonCard({ salon, showStatusBadge = true, showSpecialOffer = true, showDetailsButton = true }) {
  ...
  return (
    <Link href={`/salon/${salon.id}`} className="block h-[380px]">
      <div className="bg-white rounded-xl overflow-hidden border border-gray-100 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1 group h-full flex flex-col cursor-pointer">
        ...
        {/* Special Offer Badge */}
        {showSpecialOffer && salon.specialOffer && (
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className={`flex mt-auto ${showDetailsButton ? 'space-x-2' : ''}`}>
          {showDetailsButton && (
            <button
              onClick={(e) => {
                e.preventDefault();
