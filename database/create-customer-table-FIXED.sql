...
-- NOTE: Using quoted table names to match existing schema

-- Step 1: Create Customer table (quoted to preserve case)
...
-- Step 2: Add customer_id to Booking table (quoted table name)
...
-- Note: Existing policy might already exist, so we use IF NOT EXISTS pattern
DO $$
BEGIN
  IF NOT EXISTS (
...
DROP TRIGGER IF EXISTS trigger_customer_updated_at ON public."Customer";
...
-- SELECT COUNT(*) FROM public."Booking" WHERE customer_id IS NOT NULL;
