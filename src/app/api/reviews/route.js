...
/**
 * POST /api/reviews
 * Submit a new review (requires valid token from ReviewRequest)
 */
export async function POST(request) {
  try {
    const body = await request.json();
    const { token, shop_rating, staff_rating, comment, title } = body;
    
    if (!token) {
      return NextResponse.json(
        { success: false, error: 'Review token is required' },
        { status: 400 }
      );
    }
    ...
    if (!shop_rating || shop_rating < 1 || shop_rating > 5) {
      return NextResponse.json(
        { success: false, error: 'Shop rating must be between 1 and 5' },
        { status: 400 }
      );
    }
    ...
    const { data: review, error: reviewError } = await supabase
      ...
      .insert({
        ...
        is_verified_customer: true, // Verified via review request
    ...
    if (reviewError) {
      ...
      // Check for duplicate invoice constraint
